...
void Write_byte_18B20(uchar val)
{
	...
	//	__delay32(11);                                 //保持拉低5us
		...
		__delay32(138);                             //延时30us  64
	//	__delay32(64);                             //延时30us  64
	DQ_HIGH_HANDS();
	//	DQ_HIGH(); 
		asm("nop");                              //在两位之间插入3us延时
		asm("nop");
...
//		DQ_HIGH();                               //拉至高电平
	//	__delay32(17);							//释放总线后保持7us再读取数据
		...
		__delay32(110);							//延时64us，保证每1位的60us延时
	}
//读出温度、湿度的高8位
	for(i=8;i>0;i--)
	{
...
//		DQ_HIGH();                               //拉至高电平
	//	__delay32(17);							//释放总线后保持7us再读取数据
		...
		__delay32(110);							//延时50us，保证每1位的60us延时
	}
...
void DS18B20_err_baocun(void)
{
	uchar b20_err_count[4]={0,0,0,0};
	if(WD[0]>800)
	{
		b20_err_count[0]=HDByteReadI2C(0xa0,0x00,0xf0);
		DelayMS(5);
		b20_err_count[0]++;
		HDByteWriteI2C(0xa0,0x00,0xf0,b20_err_count[0]);
		DelayMS(5);
	}
	if(WD[1]>800)
	{
		b20_err_count[1]=HDByteReadI2C(0xa0,0x00,0xf1);
		DelayMS(5);
		b20_err_count[1]++;
		HDByteWriteI2C(0xa0,0x00,0xf1,b20_err_count[1]);
		DelayMS(5);
	}
	if(WD[2]>800)
	{
		b20_err_count[2]=HDByteReadI2C(0xa0,0x00,0xf2);
		DelayMS(5);
		b20_err_count[2]++;
		HDByteWriteI2C(0xa0,0x00,0xf2,b20_err_count[2]);
		DelayMS(5);
	}
	if(WD[3]>800)
	{
		b20_err_count[3]=HDByteReadI2C(0xa0,0x00,0xf3);
		DelayMS(5);
		b20_err_count[3]++;
		HDByteWriteI2C(0xa0,0x00,0xf3,b20_err_count[3]);
		DelayMS(5);
	}
}

void DS18B20_err_disp(void)
{
	uchar b20_count[4];
	HDSequentialReadI2C(0xa0,0x00,0xf0,b20_count,4);
	DelayMS(10);
	write_lcd_shuzi_sanwei(1,1,b20_count[0]);
	write_lcd_shuzi_sanwei(2,1,b20_count[1]);
	write_lcd_shuzi_sanwei(5,1,b20_count[2]);
	write_lcd_shuzi_sanwei(6,1,b20_count[3]);
	DelayMS(1000);
	ClrWdt();
}
